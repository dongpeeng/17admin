<template>
  <div id="banner-list">
    <el-page-header @back="$router.back()" content="关卡习题">
    </el-page-header>
...
import { insertExer, delExer, updateExer, getPassById } from '@/api/activity'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    getData(data) {
      ...
      getPassById({
        ...
        if(res.errno * 1 === 101) {
          this.listLoading = false;
          this.singExercises = res.data.singExercises;
          this.mulExercises = res.data.mulExercises;
          this.programExercises = res.data.programExercises;
        } else {
          this.$message.error(res.errmsg);
        }
      })
